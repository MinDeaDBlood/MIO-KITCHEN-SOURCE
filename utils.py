...
prog_path = getcwd() if os.name == 'nt' else os.path.normpath(os.path.abspath(os.path.dirname(sys.argv[0])))
project_name = None
...
formats = [
    ...
    (b"MZ", "exe"), (b".ELF", 'elf'),
    (b'\x7fELF', 'elf'), (b"ANDROID!", "boot"), 
    (b"VNDRBOOT", "vendor_boot"), (b'AVBf', "avb_foot"), 
    (b'BZh', "bzip2"), (b'CHROMEOS', 'chrome'), 
    (b'\x1f\x8b', "gzip"), (b'\x1f\x9e', "gzip"), 
    (b'\x02\x21\x4c\x18', "lz4_legacy"), (b'\x03\x21\x4c\x18', 'lz4'), 
    (b'\x04\x22\x4d\x18', 'lz4'), (b'\x1f\x8b\x08\x00\x00\x00\x00\x00\x02\x03', "zopfli"), 
    (b'\xfd7zXZ', 'xz'), (b']\x00\x00\x00\x04\xff\xff\xff\xff\xff\xff\xff\xff', 'lzma'), 
    (b'\x02!L\x18', 'lz4_lg'), (b'\x89PNG', 'png'), 
    (b"LOGO!!!!", 'logo', 4000), (b'\x28\xb5\x2f\xfd', 'zstd'), 
    (b'(\x05\x00\x00\$8"%', 'kdz'), (b"\x32\x96\x18\x74", 'dz'), 
    (b'\xcf\xfa\xed\xfe', 'macos_bin')
]
...
class Sdat2img:
    def __init__(self, transfer_list_file, new_data_file, output_image_file):
        ...
        if exists(self.output_image_file):
            print(f'Error: the output file "{self.output_image_file}" already exists')
            print('Remove it, rename it, or choose a different file name.')
            return

        try:
            with open(self.output_image_file, 'wb') as output_img, open(self.new_data_file, 'rb') as new_data_file:
                max_file_size = 0
                for cmd, block_list in self.list_file:
                    ...
                    for begin, block_all in block_list:
                        ...
                        # Позиция выходного файла
                        output_img.seek(begin * block_size)
                        
                        # Копирование одного блока за раз
                        while block_count > 0:
                            output_img.write(new_data_file.read(block_size))
                            block_count -= 1

                # Увеличение файла, если необходимо
                if output_img.tell() < max_file_size:
                    output_img.truncate(max_file_size)

            print(f'Done! Output image: {os.path.realpath(self.output_image_file)}')
        except IOError as e:
            print(e)
    ...
    def parse_transfer_list_file(self):
        with open(self.transfer_list_file, 'r', encoding='utf-8') as trans_list:
            ...
            if version >= 2 and (new_blocks := int(trans_list.readline())):
                trans_list.readline()  # Third line
                trans_list.readline()  # Fourth line
            yield version
            yield new_blocks
...
def gettype(file) -> str:
    ...
    for f_ in formats:
        if len(f_) == 2 and compare(f_[0]):
            return f_[1]
        elif len(f_) == 3 and compare(f_[0], f_[2]):
            return f_[1]
...
def generate_dynamic_list(dbfz, size, set_, lb, work):
    ...
    with open(os.path.join(work, "dynamic_partitions_op_list"), 'w', encoding='utf-8', newline='\n') as d_list:
        if set_ == 1:
        ...
        for part in lb:
            if set_ == 1:
                data.append(f'# Grow partition {part} from 0 to {os.path.getsize(os.path.join(work, part + ".img"))}')
                data.append(f'resize {part} {os.path.getsize(os.path.join(work, part + ".img"))}')
            elif set_ in [2, 3]:
                data.append(f'# Grow partition {part}_a from 0 to {os.path.getsize(os.path.join(work, part + ".img"))}')
                data.append(f'resize {part}_a {os.path.getsize(os.path.join(work, part + ".img"))}')
...
def v_code(num=6) -> str:
    ...
    :return:
    """
    return ''.join(str(choice([randint(0, i), chr(randint(97, 122)), chr(randint(65, 90))])) for i in range(num))
...
def img2sdat(input_image, out_dir='.', version=None, prefix='system'):
    ...
    version = version if version in versions else 4
    print("Img2sdat(1.7):" + versions[version])
    
    sparse_image = sparse_img.SparseImage(input_image, tempfile.mkstemp()[1], '0')
    block_img_diff = blockimgdiff.BlockImageDiff(sparse_image, None, version)
    block_img_diff.Compute(os.path.join(out_dir, prefix))
